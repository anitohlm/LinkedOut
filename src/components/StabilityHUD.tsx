    ...
    <motion.div
      ...
      style={{
        ...
        background: isCollapse
          ? "rgba(240,112,112,0.18)"
          : isHarmonized
          ? "rgba(126,232,225,0.14)"
          : `${tier.color}28`,
        borderBottom: `1px solid ${tier.color}${isCollapse ? "88" : "55"}`,
        backdropFilter: "blur(16px)", padding: "8px 40px",
    ...
    >
      {isCollapse ? (
        <>
          ...
          <span style={{ color: "rgba(255,255,255,0.75)", fontSize: 12 }}>New paths emerge from the wreckage — keep exploring.</span>
        </>
      ) : isHarmonized ? (
        <>
          <span style={{
          ...
          <span style={{ color: "rgba(255,255,255,0.75)", fontSize: 12 }}>{tier.effect}</span>
        </>
      ) : (
        <>
