...
export async function POST(req: NextRequest) {
  try {
    const { userMessage, speaker, allMembers, conversationHistory, isClosing, sharedMemory, timelineStability, pronouns } = await req.json();
    ...
    ) + memoryNote + pronounNote + closingNote + stabilityBehaviorNote(timelineStability ?? 100, "council") + getVoice(speaker.futureSelf?.universeId);
