...
FIRST NAME — ALWAYS derive it from "${firstName}". Phonetically sharpen "${firstName}" with hard consonants (X, Z, V, K, Y), keeping it RECOGNIZABLE but evolved — the reader should still hear "${firstName}" in it. Examples of the transformation style: Sofia→Sofyx, Marcus→Marcxen, Elena→Elynx, James→Jaxen, David→Daxid, Chris→Chryzx, Maria→Maryxa, Alex→Alxyn, Anna→Annyx, Carlos→Karlvex, Rafael→Rafaxel.
Do NOT invent an unrelated name — e.g. "${firstName}" must NOT become something like "Varkel" that shares no sounds with it. The starfaring name must clearly trace back to "${firstName}".
...
- eraName: a named historical period within that world. Invent a fresh, specific name every time — vary the format: sometimes "The [Adjective] [Noun]" (The Scorching Veil, The Iron Vigil), sometimes "[The] Age/Era/Epoch of [Thing]" (The Age of Whispering Currents), sometimes a proper-noun event name (The Sundering, The Long Silence, The Second Bloom). Never reuse the same structure twice in a row. This name must be UNIQUE to this universe — it must not duplicate any era name given to another universe in this same generation. Has nothing to do with the person's résumé.
