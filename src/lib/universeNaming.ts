...
export function nameConvention(universeId: string, firstName: string): string {
  ...
  const M: Record<string, string> = {
    ...
    cyberpunk: `Digital handles, aliases, protocol designations. May drop the human name entirely. e.g. "Architect of the Infinite Simulation", "Ghost Process 771", "${f}//Prime".`,
...
export function universeSuccess(universeId: string): string {
  const M: Record<string, string> = {
    ...
    cyberpunk: "To transcend the system entirely — to become legend in the net, untouchable, your work reshaping reality itself.",
...
export function universeFailure(universeId: string): string {
  const M: Record<string, string> = {
    ...
    cyberpunk: "Dissolution into the machine — losing your humanity to the system you tried to beat; a ghost in the code.",
